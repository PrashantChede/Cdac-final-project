...
const navigate=useNavigate();
 let [productbooksuccessfullyFlag,setProductbooksuccessfullyFlag]=useState(false);
  ...
  let [custId,setCustId]=useState(props.customerId);
  ...
  const initUser = () =>{
    customerService
      ...
      .then((response) =>{
        ...
        console.log("printing id of customer"+props.customerId);
        setCustName(response.data.firstName);
        console.warn(response.data);
      })
      .catch((error) => {
        console.log('Something went wrong', error);
...
customerService
  ...
  return (
    ...
    <div className='CustmerBody'>
      <div className='WelcomeBody'> <h1>WELCOME TO THE CUSTOMER PAGE</h1>
...
{logoutFlag?<div className='align-items-end'>
  ...
  {customerFlag?(<div className='container'>
         <div>
         <hr></hr>
         ...
         // <ProductBookSuccessfully  ></ProductBookSuccessfully>
                     <div>
                      <hr></hr>
                       <h2 align="center">!! Thanks for Availing our Service !!</h2>
                          <h3 align="center"> !! {bookMsg} </h3>
         {/* <button type="submit" id="sub" name="sub" onClick={()=>goTOCustomer()}>
          <h3 align="center">Go to CUSTOMERS HOME page</h3>
          </button> */}
